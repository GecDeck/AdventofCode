...
pub fn process(input: &str) -> u32 {
    ...
    // Get vec of seeds
    // build maps
    // send seeds through maps
    // sort vec grab first?
    println!("Getting seeds...");
    let seeds: Vec<u32> = get_seeds(input);
...
fn get_seeds(input: &str) -> Vec<u32> {
    let seeds: Vec<u32> = input.lines().next().expect("getting first line of input where all seeds are located")
        .split(":")
    ...
    for range in map_ranges {
        ...
        for line in &lines[range.0..=range.1] {
            if line.contains("map") {
                ...
                let source_range: RangeInclusive<u32> = source_range_start..=(source_range_start + range_length);
                let destination_range: RangeInclusive<u32> = destination_range_start..=(destination_range_start + range_length);
...
mod test {
    ...
    #[test]
    fn test_get_seeds() {
        assert_eq!(vec![79, 14, 55, 13], get_seeds(INPUT));
    }
}
